...
namespace sample {  // namespace helps with name resolution in reduction identity
    ...
    typedef array_type<int,5> ValueType;  // used to simplify code below
}
...
void CellCapture(int, int np, int cycle, int DecompositionStrategy, int LocalActiveDomainSize, int MyXSlices, int MyYSlices, double AConst, double BConst, double CConst, double DConst, int MyXOffset, int MyYOffset, ViewI2D ItList, ViewI NeighborX, ViewI NeighborY, ViewI NeighborZ, ViewI CritTimeStep, ViewF UndercoolingCurrent, ViewF UndercoolingChange, ViewF GrainUnitVector, ViewF CritDiagonalLength, ViewF DiagonalLength, ViewI GrainOrientation, ViewI CellType, ViewF DOCenter, ViewI GrainID, int NGrainOrientations, Buffer2D BufferA, Buffer2D BufferB, Buffer2D BufferC, Buffer2D BufferD, Buffer2D BufferE, Buffer2D BufferF, Buffer2D BufferG, Buffer2D BufferH, int BufSizeX, int BufSizeY, ViewI Locks, int ZBound_Low, int nzActive, int layernumber, ViewI LayerID, ViewI SteeringVector, ViewI numSteer_G, ViewI_H numSteer_H) {
    ...
    Kokkos::parallel_for("CellCapture", numSteer_H(), KOKKOS_LAMBDA(const int& num) {
        ...
        if (RankY == 0) {
            if (RankX == 0) {ItBounds = 5;}
            else if (RankX == MyXSlices - 1) {ItBounds = 6;}
            else {ItBounds = 1;}
        }
        else if (RankY == MyYSlices - 1) {
            if (RankX == 0) {ItBounds = 7;}
            else if (RankX == MyXSlices - 1) {ItBounds = 8;}
            else {ItBounds = 2;}
        }
        else {
            if (RankX == 0) {ItBounds = 3;}
            else if (RankX == MyXSlices - 1) {ItBounds = 4;}
            else {ItBounds = 0;}
        }
        int NListLength;
        if (ItBounds == 0) { NListLength = 26;}
        else if (ItBounds > 4) {NListLength = 11;}
        else {NListLength = 17;}
        ...
        for (int ll = 0; ll < NListLength; ll++) {
            ...
            if (MyNeighborZ < nzActive) {
                ...
                /*const int statement = (DiagonalLength(D3D1ConvPosition) >= CritDiagonalLength(26 * D3D1ConvPosition + l);
                if (statement * Kokkos::atomic_compare_exchange(&Locks(NeighborD3D1ConvPosition), 1, 0)) {}*/
