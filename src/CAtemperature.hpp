...
struct Temperature {
    ...
    void readTemperatureData(int id, Grid &grid, int layernumber) {
        ...
        if (_inputs.LayerwiseTempRead) {
            ...
            FirstLayerToRead = 0;
            LastLayerToRead = std::min(grid.NumberOfLayers, _inputs.TempFilesInSeries) - 1;
        }
        ...
        for (int LayerReadCount = FirstLayerToRead; LayerReadCount <= LastLayerToRead; LayerReadCount++) {
            ...
            parseTemperatureData(tempfile_thislayer, grid.YMin, grid.deltax, LowerYBound, UpperYBound,
        ...
        if (!(_inputs.LayerwiseTempRead)) {
            if (grid.NumberOfLayers > _inputs.TempFilesInSeries) {
                for (int LayerReadCount = _inputs.TempFilesInSeries; LayerReadCount < grid.NumberOfLayers;
                     LayerReadCount++) {
    ...
    void initialize(int id, Grid &grid, double deltat) {
    ...
    void initialize(int id, std::string SimulationType, Grid &grid, double deltat) {
    ...
    int calcMaxSolidificationEvents(Grid &grid, int NumberOfSpots, int NSpotsX, int SpotRadius, int SpotOffset) {
    ...
    void initialize(int id, Grid &grid, double FreezingRange, Inputs &inputs) {
        ...
        Kokkos::resize(LayerTimeTempHistory, grid.DomainSize, MaxNumSolidificationEvents, 3);

        // Temporary host views filled with data and copied to the device. Initialize to zeros
        view_type_float_3d_host LayerTimeTempHistory_Host("TimeTempHistory_H", grid.DomainSize,
                                                          MaxNumSolidificationEvents, 3);
        view_type_int_host NumberOfSolidificationEvents_Host("NumSEvents_H", grid.DomainSize);
    ...
    void calcMaxSolidificationEvents(int id, int layernumber, ViewI_H MaxSolidificationEvents_Host, int StartRange,
        ...
        if (layernumber > _inputs.TempFilesInSeries) {
            ...
            // Init to 0
            view_type_int_host TempMeltCount("TempMeltCount", grid.DomainSize);
            ...
            for (int i = StartRange; i < EndRange; i += 6) {
                ...
                int coord_x = getTempCoordX(i, grid.XMin, grid.deltax);
                int coord_y = getTempCoordY(i, grid.YMin, grid.deltax, grid.y_offset);
                int coord_z = getTempCoordZ(i, grid.deltax, grid.LayerHeight, layernumber, grid.ZMinLayer);
                // Convert to 1D coordinate in the current layer's domain
                int index = grid.get1Dindex(coord_x, coord_y, coord_z);
                TempMeltCount(index)++;
            }
            int MaxCount = 0;
            for (int i = 0; i < grid.DomainSize; i++) {
    ...
    // TODO: This can be performed on the device as the dirS problem is
    void initialize(int layernumber, int id, Grid &grid, double FreezingRange, int *FinishTimeStep, double deltat) {
