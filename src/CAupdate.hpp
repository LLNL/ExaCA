...
template <typename MemorySpace>
void remeltActivateCells(const int cycle, const Grid &grid, const InterfacialResponseFunction &irf, CellData<MemorySpace> &celldata, Temperature<MemorySpace> &temperature, Interface<MemorySpace> &interface) {
                            ...
                            for (int l = 0; l < 26; l++) {
                                ...
                                if (neighbor_index != -1) {
                                    ...
                                    if ((celldata.cell_type(neighbor_index) == Solid) || (coord_z == 0)) {
                                        ...
                                        // Single phase alloy or second phase transformation after the first solidification
                                        // event, set phase to primary. Otherwise, set phase to secondary
...
void fillSteeringVector(const int cycle, const Grid &grid, CellData<MemorySpace> &celldata,
    ...
    std::cout << "SV SIZE " << interface.num_steer_host(0) << std::endl;
}
