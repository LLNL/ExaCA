...
void GhostExchange(int, int, int NeighborRank_North, int NeighborRank_South, int MyXSlices,
    ...
    Kokkos::parallel_for(
        "GNInit", nzActive, KOKKOS_LAMBDA(const int &k) {
            for (int i=0; i<MyXSlices; i++) {
                // "Active" grid = only this layer (DOCenter, CaptureTimeStep), "Overall" grid = all layers (GrainID, CellType)
                int ActiveGridCellLocation_SouthBuf = k * MyXSlices * MyYSlices + i * MyYSlices + 1; // j = 1: south buffer
                int OverallCellLocation_SouthBuf = (k + ZBound_Low) * MyXSlices * MyYSlices + i * MyYSlices + 1;
                BufferSouthSend(i, k, 0) = (float)(CellType(OverallCellLocation_SouthBuf));
                BufferSouthSend(i, k, 1) = (float)(GrainID(OverallCellLocation_SouthBuf));
                BufferSouthSend(i, k, 2) = DOCenter(3 * ActiveGridCellLocation_SouthBuf + 0);
                ...
                BufferNorthSend(i, k, 1) = (float)(GrainID(OverallCellLocation_NorthBuf));
                BufferNorthSend(i, k, 2) = DOCenter(3 * ActiveGridCellLocation_NorthBuf + 0);
    ...
    while (!unpack_complete) {
        ...
        // Otherwise unpack the next buffer.
            ...
            Kokkos::parallel_for(
                "BufferUnpack", nzActive, KOKKOS_LAMBDA(const int &k) {
                    // Which rank was the data received from?
                    if ((unpack_index == 0) && (NeighborRank_South != MPI_PROC_NULL)) {
                        // this is the j = 0 data
                        int j = 0;
                        for (int i=0; i<MyXSlices; i++) {
                            int ActiveGridCellLocation = k * MyXSlices * MyYSlices + i * MyYSlices + j;
                            int OverallCellLocation = (k + ZBound_Low) * MyXSlices * MyYSlices + i * MyYSlices + j;
                            CellType(OverallCellLocation) = (int)(BufferSouthRecv(i, k, 0));
                            GrainID(OverallCellLocation) = (int)(BufferSouthRecv(i, k, 1));
                            DOCenter(3 * ActiveGridCellLocation) = BufferSouthRecv(i, k, 2);
                        ...
                        // this is the j = MyYSlices - 1 data
                        int j = MyYSlices - 1;
                        for (int i=0; i<MyXSlices; i++) {
                            int ActiveGridCellLocation = k * MyXSlices * MyYSlices + i * MyYSlices + j;
                            int OverallCellLocation = (k + ZBound_Low) * MyXSlices * MyYSlices + i * MyYSlices + j;
                            CellType(OverallCellLocation) = (int)(BufferNorthRecv(i, k, 0));
                            GrainID(OverallCellLocation) = (int)(BufferNorthRecv(i, k, 1));
                            DOCenter(3 * ActiveGridCellLocation) = BufferNorthRecv(i, k, 2);
