...
void runExaCA(int id, int np, Inputs inputs, Timers timers, Grid grid, Temperature<MemorySpace> temperature) {
    ...
    Interface<memory_space> interface(id, grid, inputs.substrate.init_oct_size);
    ...
    nucleation.placeNuclei(simulation_type, temperature, inputs.rng_seed, 0, grid, id, inputs.domain.deltat);
    ...
    for (int layernumber = 0; layernumber < grid.number_of_layers; layernumber++) {
        ...
        do {
            ...
            if (!full_domain_solidification) {
                timers.startMeltAct();
                remeltActivateCells(cycle, grid, celldata, temperature, interface);
                timers.stopMeltAct();
            }
        ...
        if (layernumber != grid.number_of_layers - 1) {
            ...
            nucleation.placeNuclei(simulation_type, temperature, inputs.rng_seed, layernumber + 1, grid, id,
