...
void PrintExaCAData(int id, int np, int nx, int ny, int nz, int MyXSlices, int MyYSlices, int ProcessorsInXDirection,
                      int ProcessorsInYDirection, ViewI_H GrainID, ViewI_H GrainOrientation, ViewI_H CritTimeStep, ViewF_H GrainUnitVector, ViewI_H LayerID, ViewI_H CellType, ViewF_H UndercoolingChange, ViewF_H UndercoolingCurrent, std::string BaseFileName, int DecompositionStrategy, int NGrainOrientations, bool *Melted, std::string PathToOutput, bool Debug, bool DebugFilesToPrint[7], bool PrintMisorientation, bool PrintFullOutput) {
    ...
    if (id == 0) {
        ...
        if (Debug) {
            // Collect data on rank 0 only for specified data structures
            if (DebugFilesToPrint[0]) CollectIntField(GrainID_WholeDomain, GrainID, nx, ny, nz, MyXSlices, MyYSlices, np, RecvXOffset, RecvYOffset, RecvXSlices, RecvYSlices, RBufSize);
            if (DebugFilesToPrint[1]) CollectIntField(LayerID_WholeDomain, LayerID, nx, ny, nz, MyXSlices, MyYSlices, np, RecvXOffset,  RecvYOffset, RecvXSlices, RecvYSlices, RBufSize);
            if (DebugFilesToPrint[2]) CollectIntField(CritTimeStep_WholeDomain, CritTimeStep, nx, ny, nz, MyXSlices, MyYSlices, np, RecvXOffset,  RecvYOffset, RecvXSlices, RecvYSlices, RBufSize);
            if (DebugFilesToPrint[3]) CollectIntField(CellType_WholeDomain, CellType, nx, ny, nz, MyXSlices, MyYSlices, np, RecvXOffset,  RecvYOffset, RecvXSlices, RecvYSlices, RBufSize);
            if (DebugFilesToPrint[4]) CollectFloatField(UndercoolingChange_WholeDomain, UndercoolingChange, nx, ny, nz, MyXSlices, MyYSlices, np, RecvXOffset,  RecvYOffset, RecvXSlices, RecvYSlices, RBufSize);
            if (DebugFilesToPrint[5]) CollectFloatField(UndercoolingCurrent_WholeDomain, UndercoolingCurrent, nx, ny, nz, MyXSlices, MyYSlices, np, RecvXOffset,  RecvYOffset, RecvXSlices, RecvYSlices, RBufSize);
            if (DebugFilesToPrint[6]) CollectBoolField(Melted_WholeDomain, Melted, nx, ny, nz, MyXSlices, MyYSlices, np, RecvXOffset,  RecvYOffset, RecvXSlices, RecvYSlices, RBufSize);
        }
        else {
            // Collect Grain ID/Melted data on rank 0
            CollectIntField(GrainID_WholeDomain, GrainID, nx, ny, nz, MyXSlices, MyYSlices, np, RecvXOffset, RecvYOffset, RecvXSlices, RecvYSlices, RBufSize);
            CollectBoolField(Melted_WholeDomain, Melted, nx, ny, nz, MyXSlices, MyYSlices, np, RecvXOffset,  RecvYOffset, RecvXSlices, RecvYSlices, RBufSize);
            if (PrintFullOutput) {
                // Collect LayerID data on rank 0 as well
                CollectIntField(LayerID_WholeDomain, LayerID, nx, ny, nz, MyXSlices, MyYSlices, np, RecvXOffset,  RecvYOffset, RecvXSlices, RecvYSlices, RBufSize);
            }
        }
        
        if (PrintMisorientation)
            PrintParaview(BaseFileName, PathToOutput, nx, ny, nz, Melted_WholeDomain, GrainID_WholeDomain, GrainOrientation, GrainUnitVector, NGrainOrientations);
        if ((PrintFullOutput)||(Debug))
            PrintParaviewGeneric(nx, ny, nz, GrainID_WholeDomain,
                                 LayerID_WholeDomain, CritTimeStep_WholeDomain, CellType_WholeDomain, UndercoolingChange_WholeDomain, UndercoolingCurrent_WholeDomain, BaseFileName, Melted_WholeDomain,
                                 PathToOutput, Debug, DebugFilesToPrint);
    }
    else {
        int SendBufSize = (MyXSlices - 2) * (MyYSlices - 2) * (nz - 2);
        if (Debug) {
            // Send data to rank 0 only for specified data structures
            if (DebugFilesToPrint[0]) SendIntField(GrainID, nz, MyXSlices, MyYSlices, SendBufSize);
            if (DebugFilesToPrint[1]) SendIntField(LayerID, nz, MyXSlices, MyYSlices, SendBufSize);
            if (DebugFilesToPrint[2]) SendIntField(CritTimeStep, nz, MyXSlices, MyYSlices, SendBufSize);
            if (DebugFilesToPrint[3]) SendIntField(CellType, nz, MyXSlices, MyYSlices, SendBufSize);
            if (DebugFilesToPrint[4]) SendFloatField(UndercoolingChange, nz, MyXSlices, MyYSlices, SendBufSize);
            if (DebugFilesToPrint[5]) SendFloatField(UndercoolingCurrent, nz, MyXSlices, MyYSlices, SendBufSize);
            if (DebugFilesToPrint[6]) SendBoolField(Melted, nz, MyXSlices, MyYSlices, SendBufSize);
        }
        else {
            ...
            if (PrintFullOutput) {
                // Send LayerID data to rank 0 as well
...
void PrintParaviewGeneric(int nx, int ny, int nz, std::vector<std::vector<std::vector<int>>> GrainID_WholeDomain, std::vector<std::vector<std::vector<int>>> LayerID_WholeDomain, std::vector<std::vector<std::vector<int>>> CritTimeStep_WholeDomain, std::vector<std::vector<std::vector<int>>> CellType_WholeDomain, std::vector<std::vector<std::vector<float>>> UndercoolingChange_WholeDomain, std::vector<std::vector<std::vector<float>>> UndercoolingCurrent_WholeDomain, std::string BaseFileName, std::vector<std::vector<std::vector<int>>> Melted_WholeDomain,
                        std::string PathToOutput, bool Debug, bool DebugFilesToPrint[7]) {
    
    std::string FName;
    if (Debug) {
        std::cout << "Printing " << DebugFilesToPrint[0] + DebugFilesToPrint[1] + DebugFilesToPrint[2] + DebugFilesToPrint[3] + DebugFilesToPrint[4] + DebugFilesToPrint[5] + DebugFilesToPrint[6] << " Paraview files post-initialization for debugging" << std::endl;
...
void PrintExaCALog(int id, int np, std::string InputFile, std::string SimulationType, int DecompositionStrategy, int MyXSlices, int MyYSlices, int MyXOffset, int MyYOffset, double Const, double BConst, double CConst, double Const, double FreezingRange, double deltax, double NMax, double dTN, double dTsigma, std::string tempfile,
                   ...
                   double FractSurfaceSitesActive, std::string PathToOutput, int NSpotsX, int NSpotsY, int SpotOffset, int SpotRadius, double InitTime, double RunTime, double OutTime) {
    ...
    if (id == 0) {
        ...
        ExaCALog << "Nucleation density was " << NMax << " x 10^12 m^-3 , mean nucleation undercooling was " << dTN << " K, and standard deviation of nucleation undercooling was " << dTN << " K" << std::endl;
        ExaCALog << "Interfacial response function parameters used were " << A << " , " << B << " , " << C << " , " << D << " , and the alloy freezing range was " << FreezingRange << std::endl;
        if (SimulationType == "C") {
            ...
            if (Remelting) ExaCALog << "Remelting was included" << std::endl;
            ...
            if (SimulationType == "R") {
                ...
                ExaCALog << "A spot pattern consisting of " << NSpotsX << " in the x direction and " << NSpotsY << " in the y direction was simulated" << std::endl;
                ExaCALog << "The spots had radii of " << SpotRadius << " cells and were " << SpotOffset << " cells apart" << std::endl;
            }
        }
        ExaCALog << "The decomposition scheme used was: " << DecompositionStrategy << std::endl;
        for (int i=0; i<np; i++) {
            ExaCALog << "Rank " << i << "contained " << XSlices[i] << " in x , " << YSlices[i] << " in y; subdomain was offset by " << XOffset[i] << " in x , " << YOffset[i] << " in y" << std::endl;
        }
        ExaCALog.close();
    }
