...
typedef Kokkos::View<char*> ViewC;
...
void TemperatureUpdate(int pid, int MyXSlices, int MyYSlices, int MyXOffset, int MyYOffset, int nz, int cycle, int &nn, double AConst, double BConst, double CConst, ViewI CritTimeStep, ViewC CellType, ViewF UndercoolingCurrent, ViewF UndercoolingChange, vector <int> NucLocI, vector <int> NucLocJ, vector <int> NucLocK, vector <int> NucleationTimes, vector <float> NucleationUndercooling, ViewI GrainID, int* GrainOrientation, ViewF DOCenter, int NeighborX[26], int NeighborY[26], int NeighborZ[26], float* GrainUnitVector, ViewI TriangleIndex, ViewF CritDiagonalLength, ViewF DiagonalLength, int NGrainOrientations);
...
void IntermediateOutputAndCheck(int pid, int cycle, int MyXSlices, int MyYSlices, int nz, int nn, int &XSwitch, ViewC::HostMirror CellType);
...
void PrintCT(int pid, int np,int nx, int ny,int nz, int MyXSlices, int MyYSlices, int ProcessorsInXDirection, int ProcessorsInYDirection, ViewC::HostMirror CellType, string BaseFileName, int DecompositionStrategy);
