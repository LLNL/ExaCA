...
void testHaloUpdate() {
    ...
    Interface<memory_space> interface(id, grid, 0.01, buf_size_initial_estimate);
...
void testResizeBuffers() {
    ...
    Interface<memory_space> interface(id, grid, 0.01, 50);
...
void testRemeltActivateCells() {
    ...
    Orientation<memory_space> orientation(id, grain_orientation_file, false, inputs.rng_seed, inputs.irf.num_phases, irf.solidificationTransformation());
    ...
    // Interface struct
    Interface<memory_space> interface(id, grid, 0.01);
