...
void PrintMisorientationData(bool* AnalysisTypes, std::string BaseFileName, int XMin, int XMax, int YMin, int YMax, int ZMin, int ZMax, int*** Melted, float*** GrainUnitVector, int*** GrainID, int NumberOfOrientations) {
    ...
    if (AnalysisTypes[0]) {
        ...
        std::cout << "Printing file " << FNameM << " of grain misorientations relative to the +Z direction, for all cells in selected volume (not including substrate)" << std::endl;
    }
        ...
        for (int i = XMin; i <= XMax; i++) {
            for (int j = YMin; j <= YMax; j++) {
                ...
                if (Melted[k][i][j] == 1) {
                    ...
                    if (AnalysisTypes[0]) MisorientationPlot << AngleZmin << std::endl;
    ...
    if (AnalysisTypes[0]) MisorientationPlot.close();
    std::cout << "Within the representative region consisting of " << (XMax-XMin+1)*(YMax-YMin+1)*(ZMax-ZMin+1) << " cells, " << NumberOfMeltedCells << " underwent melting and:" << std::endl;
    ...
    std::cout << "-- The mean misorientation relative to the +Z direction at the representative region top (Z = " << ZMax << ") is " << MisorientationSumTop/((double)(NumberOfMeltedCellsTop)) << " degrees" << std::endl;

}

//*****************************************************************************/

void PrintSizeData(bool* AnalysisTypes, std::string BaseFileName, int XMin, int XMax, int YMin, int YMax, int ZMin, int ZMax, int nx, int ny, int nz, int*** Melted, int*** GrainID, double deltax) {
    ...
    std::cout << "-- There are " << NumberOfGrains << " grains in this volume, and the mean grain volume is " << deltax * deltax * pow(10, 12) * ((double)(DomainVol)/(double)(NumberOfGrains)) << " cubic microns" << std::endl;
    std::cout << "-- The volume fraction consisting of nucleated grains is " << ((double)(NucleatedGrainCells))/((double)(DomainVol)) << std::endl;
            ...
            for (int i = XMin; i <= XMax; i++) {
                for (int j = YMin; j <= YMax; j++) {
                    ...
                    if (GrainID[k][i][j] == ThisGrainID) {
                        VolGrain[n]++;
                        if (i > TempTopX) TempTopX = i;
                        if (i < TempBottomX) TempBottomX = i;
                        if (j > TempTopY) TempTopY = j;
                        if (j < TempBottomY) TempBottomY = j;
                        if (k > TempTopZ) TempTopZ = k;
                        if (k < TempBottomZ) TempBottomZ = k;
                    }
                }
            }
        }
    ...
    std::cout << "-- The mean grain aspect ratio (Z direction to transverse) is " << ARSum/(float)(NumberOfGrains) << std::endl;
    std::cout << "-- The mean volume-weighted grain aspect ratio (Z direction to transverse) is " << VolWtARSum/((float)(DomainVol)) << std::endl;
    ...
    if (AnalysisTypes[1]) {
        ...
        std::cout << "Printing file " << FNameV << " of grain volumes (in cubic microns) in selected volume" << std::endl;
        for (int n=0; n<NumberOfGrains; n++) {
    ...
    if (AnalysisTypes[5]) {
        ...
        std::cout << "Printing file " << FNameGH << " of mean height distribution (in microns) for the selected volume" << std::endl;
...
void PrintGrainAreaData(bool* AnalysisTypes, std::string BaseFileName, double deltax, int XMin, int XMax, int YMin, int YMax, int ZMin, int ZMax, int*** GrainID) {
    ...
    if (AnalysisTypes[3]) {
        std::cout << "Printing file " << FName1 << " of grain area values (in square microns) for all Z coordinates" << std::endl;
        Grainplot1.open(FName1);
    }
    if (AnalysisTypes[4]) {
        std::cout << "Printing file " << FName2 << " of weighted grain area values (in square microns) for every 5th Z coordinate" << std::endl;
        Grainplot2.open(FName2);
    }
    
    if (!(AnalysisTypes[2])&&(!(AnalysisTypes[3]))) ZMin = ZMax; // only print grain area/weighted grain area to screen at this one Z coordinate
    ...
    for (int k = ZMin; k <=ZMax; k++) {
        ...
        if (k == ZMax) {
            std::cout << "Number of grains at the representative region top (Z = " << ZMax << "): " << GrainsThisLayer << std::endl;
            if (AnalysisTypes[6]) {
                std::string FName3 = BaseFileName + "_GrainWidthDistribution.csv";
                std::ofstream Grainplot3;
                std::cout << "Printing file " << FName3 << " of grain width distribution (in microns) at Z = " << ZMax << std::endl;
        ...
        if (((AnalysisTypes[3])&&(k % 5 == 0))||(k == ZMax)) {
            ...
            if (AnalysisTypes[3]) Grainplot2 << WeightedArea * deltax * deltax / pow(10, -12) << std::endl;
            if (k == ZMax) std::cout << "-- The mean weighted grain area at the representative region top (Z = " << ZMax << ") is " << WeightedArea * deltax * deltax / pow(10, -12) << " square microns" << std::endl;
        }
        if (AnalysisTypes[4]) Grainplot1 << MeanGrainAreaThisLayer * deltax * deltax / pow(10, -12) << std::endl;
        if (k == ZMax) std::cout << "-- The mean grain area at the representative region top (Z = " << ZMax << ") is " << MeanGrainAreaThisLayer * deltax * deltax / pow(10, -12) << " square microns" << std::endl;
    }
    if (AnalysisTypes[3])  Grainplot1.close();
    if (AnalysisTypes[4])  Grainplot2.close();
    
}
                              
//*****************************************************************************/
void PrintPoleFigureData(bool* AnalysisTypes, std::string BaseFileName, int NumberOfOrientations, float**, int XMin, int XMax, int YMin, int YMax, int ZMin, int ZMax, int*** GrainID, int*** Melted) {
...
//                    PyEBSDPlot << XMultiplier*(i-XMin) << "," << j-YMin << " " << GrainEulerAngles[GOVal][0] << " " << GrainEulerAngles[GOVal][1] << " " << GrainEulerAngles[GOVal][2] << std::endl;
//                }
//            }
//        }
//        PyEBSDPlot.close();
...
//*****************************************************************************/
void PrintInversePoleFigureCrossSections(int NumberOfCrossSections, std::string BaseFileName, std::vector <int> CrossSectionPlane, std::vector <int> CrossSectionLocation, int nx, int ny, int nz, int NumberOfOrientations, int*** GrainID, float**) {
    ...
    for (int n=0; n<NumberOfCrossSections; n++) {
        ...
        std::string CSType;
        if (CrossSectionPlane[n] == 0) CSType = "XZ";
        else if (CrossSectionPlane[n] == 1) CSType = "YZ";
        else CSType = "XY";
//        pyEBSD print routines in progress
//        std::string FNameCS1 = BaseFileName + std::to_string(n) + " " + CSType + "pyEBSDCrossSection.ang";
//        std::cout << "Cross-section number " << n+1 << " being printed to file " << FNameCS1 << " for pyEBSD" << std::endl;
//        std::ofstream GrainplotCS1;
//        GrainplotCS1.open(FNameCS1);
//        GrainplotCS1.close();
        
        std::string FNameCS2 = BaseFileName + "-" + std::to_string(n) + "_" + CSType + "MTEXCrossSection.txt";
        std::cout << "Cross-section number " << n+1 << " being printed to file " << FNameCS2 << " for MTEX" << std::endl;
...
void PrintExaConstitRVEData(int NumberOfRVEs, std::string BaseFileName, int nx, int ny, int nz, double deltax, int*** GrainID, std::vector <int> XLow_RVE, std::vector <int> XHigh_RVE, std::vector <int> YLow_RVE, std::vector <int> YHigh_RVE, std::vector <int> ZLow_RVE, std::vector <int> ZHigh_RVE) {
    ...
    for (int n=0; n<NumberOfRVEs; n++) {
        ...
        std::cout << "RVE number " << n+1 << " with X coordinates " << XLow_RVE[n] << "," << XHigh_RVE[n] << "; Y coordinates " << YLow_RVE[n] << "," << YHigh_RVE[n] << "; Z coordinates " << ZLow_RVE[n] << "," << ZHigh_RVE[n] << " being printed to file " << FNameE << " for ExaConstit" << std::endl;
        std::ofstream GrainplotE;
        GrainplotE.open(FNameE);
        GrainplotE << "Coordinates are in CA units, 1 cell = " << deltax << " m. Data is cell-centered. Origin at " << XLow_RVE[n] << "," << YLow_RVE[n] << "," << ZLow_RVE[n] << " , domain size is " << XHigh_RVE[n]-XLow_RVE[n]+1 << " by " << YHigh_RVE[n]-YLow_RVE[n]+1  << " by " << ZHigh_RVE[n]-ZLow_RVE[n]+1  << " cells" << std::endl;
